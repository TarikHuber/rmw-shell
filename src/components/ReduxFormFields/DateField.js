...
class DateField extends Component {
  ...
  render() {
    const { disabled, input, floatingLabelText, datePickerText, theme, intl } = this.props
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'flex-end' }}>
        ...
        <IconButton
          onClick={() => {
            this.refs[name].getRenderedComponent().refs.component.openDialog()
          }}
          tabIndex={-1}
          disabled={disabled}
          tooltip={datePickerText}
        >
          <Icon className="material-icons" style={{ fontSize: 12 }} color={theme.palette.primary1Color}>
            event
          </Icon>
        </IconButton>
      </div>
